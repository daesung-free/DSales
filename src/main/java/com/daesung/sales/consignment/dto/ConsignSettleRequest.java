...
public record ConsignSettleRequest(
    ...
    public record Settlement(
            ...
            @Schema(description = """
                    ...
                    ★정산수량과 반품수량의 합이 미결 잔여를 넘어도 **차단하지 않는다** —
                    발주처 확정(2026-08-31)에 따라 응답 `overWarning`에 경고만 담고 등록은 진행한다
                    (잔여가 음수로 남고 담당자가 수기로 정리한다).
                    ‼️예전 설명은 "초과 시 409"였는데, 차단을 걷어낸 뒤에도 문구가 남아 있었다.
                    화면이 이 설명을 보고 입력을 막으면 8/31 확정이 되돌아간다.""", example = "300")
            ...
            @Schema(description = """
                    ...
                    ★정산수량과 마찬가지로 미결 잔여를 넘어도 차단하지 않는다 — `overWarning`에 경고만.""",
                    example = "20")
