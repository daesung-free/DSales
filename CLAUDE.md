...
- **✅테스트(회귀 방어)**: 통합테스트 **34건**(Testcontainers MySQL8+Redis, `IntegrationTestSupport` 베이스) — 매출리포트/세무 13, 마스터·위탁·단가 11, 인증·RBAC 3, **재고엔진(이고·폐기·실사·BOM조립해체)·매출취소 역분개·월마감 잠금 6**, 앱 1. `./gradlew test` → 34통과. ⚠️ Docker 29 대응 위해 TC **1.21.3** override(build.gradle). ⏳미커버(백로그): 수금/채권·대시보드·DSRE(off).

**⬜ 미구현(결정 없이 가능)**: ~~정산내역서~~ **✅구현(위탁 회계기준 확정 확인)** / 리포트 데이터 API 잔여(응시·회차현황=DSRE 의존) / 매출액명세서 등 리포트의 **프론트 RDLC 렌더링(나눔고딕, 백엔드 밖)** / 마이그레이션 스크립트(Phase6)

**⏸ 회신 대기(발주처·재무팀)**: ~~상태머신(상태전이도 승인=하드블로커)~~ **✅정정: DSRE2 단일축 STATE = DSRE 소관, 우리 재구축 범위 아님(§4 로직C) — 블로커 아님** / **catCode 데이터 출처·연도 정의(이슈2·3 — 리포트 5종 급소: 레거시는 catCode에 연도 인코딩, 우리는 자유문자열)** / 위탁 회계기준 적정성 / ~~매입원가 출처~~ **✅해소(=매입입고 unit_cost, 8p 구분값→16p 자동연결, 재무팀 확정 2026-07-28)** — 남은 건 이슈#57(외부콘텐츠 매입액B가 매입반품 차감 순액인지, 연말엣지) / 여신한도(별개?) / 수금 건별매칭 / **법인 축(멀티테넌트: 현재 단일법인 가정 — 월마감·계산서·거래명세서 공급자)** / 세금 반올림(현재 버림) / 취소 30% 재분류(우리 모델이 이미 커버).
