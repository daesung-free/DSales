...
- **착수 우선순위(전부 회신 대기 없이 지금 가능 — 양식·레거시 대조 완료로 추가1도 착수 가능해짐)**: ~~추가3(거래처 컬럼)~~ **✅완료(커밋 0f000bf, V20)** →~~추가4(조회 반영)~~**✅완료(SaleResponse에 partnerCityName·partnerName1)**→~~추가2(반품 교재식)~~**✅완료(GET /sales/returnable + return-inbound 범위검증, RETURN_EXCEEDS)**→추가1(매출 업로드: 학교·회차 필드 복원 + 파서). 남은 1건 되물어볼 것 없음.
