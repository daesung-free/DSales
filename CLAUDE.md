...
- **엑셀 다운로드(아마란스 연동 대신, 재무팀 확정)**: 범용 `ExcelExportUtil`(Jackson 평탄화+POI, `Col(한글헤더,필드)` 스펙=드라이브 파일 형식). `.../export` 엔드포인트 — 매출액명세서·37p월별매출액명세서·순매출조회(`/sales/*/export`) / 수익신고·38p계산서월별신고(`/closing/*/export`). ⚠️나머지 리포트(과목별·도서입출고·매출대비표·10일분기·담보만기·마스터목록)는 동일 패턴으로 확장 예정.
