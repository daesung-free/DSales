...
- **엑셀 다운로드(아마란스 연동 대신, 재무팀 확정)**: 범용 `ExcelExportUtil`(Jackson 평탄화+POI, `Col(한글헤더,필드)`=드라이브 파일 형식). **전 리포트·마스터에 `.../export`**: 매출(매출액명세서·37p·순매출조회·과목별·도서입출고·매출대비표) / 마감(수익신고·38p·10일분기) / 마스터(담보만기·거래처·도서·창고 목록). 파일명 한글 UTF-8. (계산서신고 홈택스 xlsx는 별도 전용 export)
